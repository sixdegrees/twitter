  ...
  class Client
    ...
    module Tweets
      ...
      def status(id, options={})
        response = get("1.1/statuses/show/#{id}", options)
      ...
      def update(status, options={})
        response = post('1.1/statuses/update', options.merge(:status => status))
        format.to_s.downcase == 'xml' ? response['status'] : response
      end
      ...
      def update_with_media(status, image, options={})
        response = post('1.1/statuses/update_with_media', options.merge('media[]' => image, 'status' => status))
        format.to_s.downcase == 'xml' ? response['status'] : response
      end
      ...
      def status_destroy(id, options={})
        response = delete("1.1/statuses/destroy/#{id}", options)
        format.to_s.downcase == 'xml' ? response['status'] : response
      end
      ...
      def retweet(id, options={})
        response = post("1.1/statuses/retweet/#{id}", options)
        format.to_s.downcase == 'xml' ? response['status'] : response
      end
      ...
      def retweets(id, options={})
        response = get("1.1/statuses/retweets/#{id}", options)
      ...
      def retweeters_of(id, options={})
        ...
        response = get("1.1/statuses/#{id}/retweeted_by#{'/ids' if ids_only}", options)
